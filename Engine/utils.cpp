...
#pragma region ArraysFunctionality
...
#pragma endregion ArraysFunctionality

#pragma region MathFunctionality

float utils::RadToDegree(const float radian)
{
	return radian * 180.f / utils::g_Pi;
}

float utils::DegreeToRad(const float degree)
{
	return degree * utils::g_Pi / 180.f;
}
#pragma endregion CollisionFunctionality

#pragma region ColorFunctionality
Color4f utils::RgbIntToColor(int r, int g, int b, float a)
{
	const float diviser{ 255.f };
	return Color4f{ r / diviser, g / diviser,b / diviser,a };
}

#pragma endregion ColorFunctionality

#pragma region PositionFunctionality

Point2f utils::GetCenterOfRectangle(const Rectf& rect)
{
	return GetCenterOfRectangle(rect.left, rect.bottom, rect.width, rect.height);
}

Point2f utils::GetCenterOfRectangle(const float left, const float bottom, const float width, const float height)
{
	return Point2f{ left + width / 2, bottom + height / 2 };
}

Point2f utils::GetCenterOfRectangle(const Point2f bottomLeft, const float width, const float heigth)
{
	return GetCenterOfRectangle(bottomLeft.x, bottomLeft.y, width, heigth);
}

Point2f utils::GetElementCenterOfRectangle(const Rectf& rect, const Texture& text)
{
	const float resultx{ (rect.width / 2 - text.GetWidth() / 2) };
	const float resulty{ (rect.height / 2 - text.GetHeight() / 2) };
	const Point2f test{ rect.left + (rect.width / 2 - text.GetWidth() / 2), rect.bottom + (rect.height / 2 - text.GetHeight() / 2) };
	return Point2f{ rect.left + (rect.width / 2 - text.GetWidth() / 2), rect.bottom + (rect.height / 2 - text.GetHeight() / 2) };
}

Point2f utils::GetElementCenterOfRectangle(const Rectf& rect, const Rectf& element)
{
	const float resultx{ (rect.width / 2 - element.width / 2) };
	const float resulty{ (rect.height / 2 - element.height / 2) };
	return Point2f{ rect.left + (rect.width / 2 - element.width / 2), rect.bottom + (rect.height / 2 - element.height / 2) };
}

#pragma endregion PositionFunctionality
